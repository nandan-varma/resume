...
The editor uses `texlyre-busytex` — a WASM LaTeX compiler. Do not import it statically — it is lazy-loaded and cached via refs (`use-engine.ts`). `busytex.wasm`, `busytex.js`, and `texlive-extra.data` are all redirected in `next.config.ts`. Each compile injects a `\typeout{JOBMATCH_FILL:...}` probe right before `\end{document}` to read `\pagetotal`/`\textheight` off the compile log — this gives `fillRatio` (how full the last page is), passed alongside `pageCount` to the AI so it can distinguish "one page but nearly empty, could add more" from "one page and well-used" rather than just a page count.
