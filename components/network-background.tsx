...
const _bufs      = Array.from({ length: BUCKETS }, () => new Float32Array(MAX_EDGES * 4))
...
export function NetworkBackground() {
  ...
  useEffect(() => {
    ...
    const nodes: Node[] = Array.from({ length: targetCount(W, H) }, () => spawnNode(W, H))
    ...
    const onResize = () => {
      ...
      for (const n of nodes) { n.x = (n.x / pW) * W; n.y = (n.y / pH) * H }
      // Grow/shrink node pool to match new viewport density.
      const next = targetCount(W, H)
      while (nodes.length < next) nodes.push(spawnNode(W, H))
      nodes.length = next
      applySize()
    }
    ...
    const tick = () => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if      (nd.x < 0) { nd.x = 0; nd.vx = -nd.vx }
        else if (nd.x > W) { nd.x = W; nd.vx = -nd.vx }
        if      (nd.y < 0) { nd.y = 0; nd.vy = -nd.vy }
        else if (nd.y > H) { nd.y = H; nd.vy = -nd.vy }
      }
      ...
      ctx.lineWidth    = 4
      ctx.globalAlpha  = dark ? 0.048 : 0.030
      ctx.beginPath()
      for (let b = 0; b < BUCKETS; b++) {
        const c = _lens[b]; const buf = _bufs[b]
        for (let e = 0; e < c; e++) {
          const o = e << 2
      ...
      for (let b = 0; b < BUCKETS; b++) {
        const c = _lens[b]; if (c === 0) continue
      ...
      ctx.globalAlpha = dark ? 0.10 : 0.06
      ctx.beginPath()
      for (let i = 0; i < n; i++) {
